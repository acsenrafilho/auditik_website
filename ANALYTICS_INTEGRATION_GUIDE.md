...
- **Lead conversions** fire on [`/obrigado/`](pages/obrigado.tsx) only after CRM accepts the lead (`lib/thank-you.ts` session token): `trackMetaLead` → dataLayer `meta_lead` (Meta Pixel via GTM-NVWQ3PF2 Custom Event → standard Lead) plus `trackConversion` → Google Ads. Direct visits to `/obrigado/` redirect without conversion events.
